...
for i in range(1,length(orders))
    ...
    try
    xf, _, _, _, _ = adaptive_block_coordinate_frank_wolfe(
        f,
        grad!,
        prod_lmo.lmos,
        x0;
    ...
    export_data((gaps[i],), (labels_filename[i],), filename_prefix=string("BCFW_adpative_A",num_trials,"_",n,"_"),filename_suffix=filename_suffix,compute_FWgaps=compute_FWgaps,iter_skip=convert(Int,ceil(max_iters[i]/100)))
    println("Done.")
end
